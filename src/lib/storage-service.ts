...
/** Uploads the store's static payment QR code to the shared public image bucket. */
...
export async function deletePaymentQrImage(imageUrl: string): Promise<void> {
  await deleteMenuImage(imageUrl);
}

/** Uploads a loyalty reward photo to the shared public image bucket. */
export async function uploadRewardImage(file: File): Promise<string> {
  ...
  const supabase = getSupabaseAdmin();
  const ext = file.name.split('.').pop() ?? 'jpg';
  const path = `reward-images/${crypto.randomUUID()}.${ext}`;
  const { error } = await supabase.storage.from(BUCKET).upload(path, file, {
    contentType: file.type,
    upsert: false,
  });
